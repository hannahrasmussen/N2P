...
import Differential_Equation_Solver_new as DES
...
#@nb.jit(nopython=True)
def N_eff(T_i,a_i,a_f,y_final,e_a,bxsz=1): #this code assumes that y_final does not have both neutrino & antineutrino information
    TCM = T_i*a_i/a_f 
    e_dens = (TCM**4/(2*np.pi**2))*newtrapezoid(y_final[:int((len(y_final)-3))]*e_a**3,e_a)
    neff = e_dens/((7/4)*(4/11)**(4/3)*(np.pi**2/30)*(y_final[-2])**4)
...
def driver(a_array, T_array, y_mat, e_matrix, ms, mixangle):
    y_matrix = y_mat #np.transpose(y_mat) #dQda, Temp, and time are y_matrix[-3], y_matrix[-2],
